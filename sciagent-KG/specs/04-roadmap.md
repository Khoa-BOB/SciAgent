...
4. **Shipped**: acronym-detection fallback in `cluster_names()`
   (`resolve.py`) alongside the embedding-similarity check — a name joins an
   existing cluster if its letters form an acronym of the other name's
   initials (in either direction), checked only after cosine similarity
   already failed. See `_initials`/`_is_acronym_token`/`_acronym_fallback_match`
   in `resolve.py` and `tests/test_resolve.py` for unit coverage (fake
   embedding model — no real model download or corpus needed). **Still
   open**: the directly-checkable verification step this item originally
   called for — rerun `resolve` + `merge` against the real corpus, then
   `cli.py entities` again, and confirm merge recall on the 3 known
   acronym/expansion pairs actually moved off 0%. Not done here because it
   needs a live Neo4j with the real corpus loaded, which this change was
...
implemented; #5 is still a local, free fix to make, verifiable against the
new benchmark without touching the LLM.
